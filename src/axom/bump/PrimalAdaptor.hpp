 ...
 * \tparam MAX_VERTS The maximum number of vertices allowed in a polygon.
 * \tparam makeFaces Whether to make faces for polyhedral shapes or to make primal::Polyhedron.
 */
template <typename TopologyView, typename CoordsetView, int MAX_VERTS = 12, bool makeFaces = false>
struct PrimalAdaptor
{
  using value_type = typename CoordsetView::value_type;
  using Polygon =
    axom::primal::Polygon<value_type, CoordsetView::dimension(), axom::primal::PolygonArray::Static, MAX_VERTS>;
