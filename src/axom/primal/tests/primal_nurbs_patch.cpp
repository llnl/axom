...
//------------------------------------------------------------------------------
TEST(primal_nurbspatch, nurbs_parameter_expansion)
{
  SLIC_INFO("Testing NURBS Patch parameter space expansion");
  ...
  // Expand the parameter space of the patch
  constexpr double buffer = 0.1;
  supPatch.expandParameterSpace(buffer);
  ...
  // Check the parameter space of the superpatches' knots
  EXPECT_NEAR(supPatch.getMinKnot_u(), min_u - buffer, 1e-10);
  EXPECT_NEAR(supPatch.getMaxKnot_u(), max_u + buffer, 1e-10);
  EXPECT_NEAR(supPatch.getMinKnot_v(), min_v - buffer, 1e-10);
  EXPECT_NEAR(supPatch.getMaxKnot_v(), max_v + buffer, 1e-10);

  // Check that the patches are equal in the original parameter space
  constexpr int npts = 15;
  double u_pts[npts], v_pts[npts];
  axom::numerics::linspace(min_u - buffer, max_u + buffer, u_pts, npts);
  axom::numerics::linspace(min_v - buffer, max_v + buffer, v_pts, npts);
