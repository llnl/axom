 ...
 * 
 * Scalar-field line integrals and scalar-field area integrals are of the form 
                                     ...
                                     int npts_P = 0)
{
  ...
  const auto lower_bound_y = detail::curve_array_lower_bound_y(carray);

  for(int i = 0; i < carray.size(); ++i)
  {
    total_integral += detail::evaluate_area_integral_component(carray[i],
                                                               std::forward<Lambda>(integrand),
                                                               lower_bound_y,
                                                               npts_Q,
                                                               npts_P);
