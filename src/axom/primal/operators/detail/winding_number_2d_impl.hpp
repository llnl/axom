 ...
 *  i.e. the query it outside its bounding box, at which point a direct formula for the GWN 
                             ...
                             double EPS = 1e-8)
{
  ...
  {
    isConvexControlPolygon = is_convex(Polygon<T, 2>(bezier.getControlPoints()), EPS);
  }

  if(isConvexControlPolygon &&
     (squared_distance(q, bezier[0]) <= edge_tol * edge_tol ||
                            ...
                            double EPS = 1e-8)
{
  ...
  {
    return detail::linear_winding_number(q, nurbs[0], nurbs[nurbs.getNumControlPoints() - 1], isOnCurve, edge_tol);
  }
