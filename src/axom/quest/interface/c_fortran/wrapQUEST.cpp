...
int QUEST_inout_init_mpi(const char *fileName, MPI_Fint comm)
{
    ...
    int SHC_rv = axom::quest::inout_init(SHC_fileName_cxx, SHCXX_comm);
    return SHC_rv;
...
int QUEST_inout_init_serial(const char *fileName)
{
    ...
    const std::string SHC_fileName_cxx(fileName);
    int SHC_rv = axom::quest::inout_init(SHC_fileName_cxx);
    return SHC_rv;
...
int QUEST_signed_distance_init_mpi(const char *file, MPI_Fint comm)
{
    ...
    MPI_Comm SHCXX_comm = MPI_Comm_f2c(comm);
    int SHC_rv = axom::quest::signed_distance_init(SHC_file_cxx, SHCXX_comm);
    return SHC_rv;
...
int QUEST_signed_distance_init_mpi_bufferify(char *file, int SHT_file_len, MPI_Fint comm)
{
    ...
    const std::string SHC_file_cxx(file, SHC_file_trim);
    MPI_Comm SHCXX_comm = MPI_Comm_f2c(comm);
    int SHC_rv = axom::quest::signed_distance_init(SHC_file_cxx, SHCXX_comm);
    return SHC_rv;
...
int QUEST_signed_distance_init_serial(const char *file)
{
    ...
    const std::string SHC_file_cxx(file);
    int SHC_rv = axom::quest::signed_distance_init(SHC_file_cxx);
    return SHC_rv;
...
void QUEST_signed_distance_set_execution_space_bufferify(int execSpace)
{
    // splicer begin function.signed_distance_set_execution_space_bufferify
    axom::quest::SignedDistExec SHCXX_execSpace = static_cast<axom::quest::SignedDistExec>(execSpace);
    axom::quest::signed_distance_set_execution_space(SHCXX_execSpace);
    // splicer end function.signed_distance_set_execution_space_bufferify
}
...
double QUEST_signed_distance_evaluate_1(double x, double y, double z, double *cp_x, double *cp_y, double *cp_z, double *n_x, double *n_y, double *n_z)
{
    ...
    double SHC_rv = axom::quest::signed_distance_evaluate(x, y, z, *cp_x, *cp_y, *cp_z, *n_x, *n_y, *n_z);
    return SHC_rv;
