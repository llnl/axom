   ...
   """
    ...
    def write(self, f):
        ...
        while i < self.vertCount:
            f.write(
                f"{self.points[i][0]} {self.points[i][1]} {self.points[(i+1)][0]} {self.points[(i+1)][1]}\n"
            )
            i = i + 2
        i = 0
        while i < self.vertCount:
            f.write(
                f"{self.points_ho[i][0]} {self.points_ho[i][1]} {self.points_ho[(i+1)][0]} {self.points_ho[(i+1)][1]}\n"
            )
            i = i + 2
...
def add_circle(data, elem, origin, radius, reverse=False):
    ...
    segments = (  # segment 0
        ((origin[0] + p0[0], origin[1] + p0[1]), (origin[0] + p1[0], origin[1] + p1[1]),
         (origin[0] + p2[0], origin[1] + p2[1]), (origin[0] + p3[0], origin[1] + p3[1])),
        # segment 1
        ((origin[0] - p3[0], origin[1] + p3[1]), (origin[0] - p2[0], origin[1] + p2[1]),
         (origin[0] - p1[0], origin[1] + p1[1]), (origin[0] - p0[0], origin[1] + p0[1])),
        # segment 2
        ((origin[0] - p0[0], origin[1] - p0[1]), (origin[0] - p1[0], origin[1] - p1[1]),
         (origin[0] - p2[0], origin[1] - p2[1]), (origin[0] - p3[0], origin[1] - p3[1])),
        # segment 3
...
def open_polygons():
    ...
    for j in range(n):
        for i in range(n):
            ...
            dy = (ext[3] - ext[2]) / n
            margin = dx * 0.05
            e = (ext[0] + i * dx + margin, ext[0] + (i + 1) * dx - margin, ext[2] + j * dy + margin,
                 ext[2] + (j + 1) * dy - margin)
