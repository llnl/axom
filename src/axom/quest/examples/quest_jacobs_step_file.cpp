...
void graphical_abstract_exploded()
{
  ...
  axom::primal::exportSliceScalarFieldToVTK<double>(prefix + filename + "_slice2.vtk",
                                                    wn_field,
                                                    origin,
                                                    axom::primal::Vector<double, 3> {0.3, 1, 0},
                                                    axom::primal::Vector<double, 3> {0, 0, 1},
                                                    the_range,
                                                    the_range,
                                                    200,
...
void quadrature_on_sphere()
{
  ...
  // (!bBox, !oBox, casting, noCache)
  int case_code = -1;
  auto wn_field_surface_quad_nonadaptive =
    [&sphere_faces, &edge_tol, &ls_tol, &EPS, &case_code](
    ...
    std::pair<double, double> result = {wn, node_count};
    return result;
  };

  auto wn_field_surface_quad = [&sphere_faces, &edge_tol, &ls_tol, &EPS, &case_code](
                                 axom::primal::Point<double, 3> query) -> std::pair<double, double> {
    int node_count = 0;
  ...
  auto wn_field_boundary_quad_nonadaptive =
    [&nurbs_faces_data, &edge_tol, &ls_tol, &EPS, &case_code](
      axom::primal::Point<double, 3> query) -> std::pair<double, double> {
    double wn = 0.0;
  ...
  auto wn_field_boundary_quad = [&nurbs_faces_data, &edge_tol, &ls_tol, &EPS, &case_code](
                                  axom::primal::Point<double, 3> query) -> std::pair<double, double> {
    double wn = 0.0;
    int integrated_trimming_curves = 0;
    int node_count = 0;
  ...
  auto wn_field_exact = [&nurbs_faces_data, &edge_tol, &EPS, &case_code](
                          axom::primal::Point<double, 3> query) -> double {
    int integrated_trimming_curves;
  ...
  //   prefix + "slice1_boundary_nonadaptive.vtk",
  //   wn_field_boundary_quad_nonadaptive,
  //   axom::primal::Point<double, 3> {-0.4, -0.4, -0.75},
  //   axom::primal::Vector<double, 3> {0, 1, 0},
  //   axom::primal::Vector<double, 3> {1, 0, 0},
  //   0.8,
  ...
  // axom::primal::exportSplitScalarSliceFieldToVTK<double>(
  //   prefix + "slice2_boundary_nonadaptive.vtk",
  //   wn_field_boundary_quad_nonadaptive,
  //   axom::primal::Point<double, 3> {-0.4, -0.4, -0.75},
  //   axom::primal::Vector<double, 3> {0, 0, 1},
  //   axom::primal::Vector<double, 3> {1, 1.2, 0},
...
void plot_trimming_curves()
{
  ...
  std::string filename = "machine_part_ugly_tims";
  auto stepProcessor = import_step_file(prefix, filename, true, 0.005, 0.0025);
...
void generic_direction_timing_test(std::string prefix,
                                   std::string filename,
                                   std::string file_tag,
...
void spring_direction_example()
{
  ...
  axom::primal::exportSplitScalarSliceFieldToVTK<double>(
    prefix + filename + "_slice_random.vtk",
    wn_random_field,
    axom::primal::Point<double, 3> {-0.005, 0, 0},
    axom::primal::Vector<double, 3> {0, 1, 0},
    axom::primal::Vector<double, 3> {0, 0, 1},
    meshBBox.getMax()[1] - meshBBox.getMin()[1],
    meshBBox.getMax()[2] - meshBBox.getMin()[2],
 ...
 * @return A vector of sample points near unit sphere
 */
std::vector<axom::primal::Point<double, 3>> generateSamplePointsOnBBox(
  axom::primal::BoundingBox<double, 3> bbox,
  int numSamples)
{
  std::vector<axom::primal::Point<double, 3>> samplePoints;
  ...
  SLIC_INFO(
    axom::fmt::format("Generating {} sample points in bounding box with range: "
                      "({}, {}) x ({}, {}) x ({}, {})",
                      numSamples,
                      bbox.getMin()[0],
                      bbox.getMax()[0],
  ...
  for(int i = 0; i < numSamples; ++i)
  {
    ...
    if(useFixedPoint)
      ...
      samplePoints.emplace_back(axom::primal::Point<double, 3> {x0, y0, z0});
  }
...
void test_rotate_curve()
{
  ...
  sphere_patch[2] = axom::primal::Point<double, 2> {1.0, 0.0};

  sphere_patch.makeRational();
  sphere_patch.setWeight(1, 1.0 / std::sqrt(2.0));

  auto sphere_patch_patches = rotate_curve(sphere_patch);
  ...
  //     for(int i = 0; i < 4; ++i)
  //     {
  //       auto point = sphere_patch_patches[i].evaluate(u, v);
  ...
  const auto& query = axom::primal::Point<double, 3> {0.5, 0.5, -1.6};
  ...
  else
  {
    ...
    std::cout << std::sqrt(query[0] * query[0] + query[1] * query[1] +
                                         ...
                                         const std::string& out_suffix = "")
{
  ...
  for(long i = 0; i < 1e7; ++i)
  {
    ...
    {
      // If we miss the point, we try to run it with a higher precision in the intersection routine
      double high_gwn = 0.0;
      for(int i = 0; i < 8; ++i)
      {
      ...
      // If we get a half-integer value, then we know that the intersectio routine failed twice,
      ...
      // If we get the right answer at that point, then we know it was the intersection that failed.
                            ...
                            const std::string& out_suffix = "")
{
  ...
  true_bbox.addPoint(axom::primal::Point<double, 3> {0.0, 0.0, 0.0});
  true_bbox.addPoint(axom::primal::Point<double, 3> {1.0, 1.0, 1.0});
  ...
  while(true)
  {
    ...
    // Only check points that are within 0.01 of the bounding box
    if((x0 > 0.0 + buffer && x0 < 1.0 - buffer) && (y0 > 0.0 + buffer && y0 < 1.0 - buffer) &&
       (z0 > 0.0 + buffer && z0 < 1.0 - buffer))
    {
      continue;
    }
    ...
    int rounded_gwn = static_cast<int>(std::round(gwn));
    int expected_gwn = true_bbox.contains(query);
...
void query_timing_test(const std::string& test_prefix,
                       const std::string& out_prefix,
                       const std::vector<axom::primal::Point<double, 3>>& query_points,
                       const std::string& out_suffix = "")
{
  ...
  for(int idx = 0; idx < num_query_pts; ++idx)
  {
    ...
    const auto& query = query_points[idx];
    // const auto& query = axom::primal::Point<double, 3> {0.694813608972052, -0.218268589793227, 0.685266991761081};
    int case_code = -1;
    axom::utilities::Timer timer(false);
...
int main()
{
  ...
  //   int num_query_pts = 10000;
  //   double distance_to_surface = 1e-6;
  //   std::string test_prefix = "less_random";
  ...
  //   query_timing_test(test_prefix, out_prefix,
  //                     query_pts,
  //                     axom::fmt::format("_within_{}", distance_to_surface));

  // Points to reconsider for the teardrop
  // 0.635230868129530, 0.771939455645892, -0.971960652699671
  // 0.630155969803398, -0.660230662030716, 0.020699507267887
  // -0.932162221800155, 0.024024984192783, -0.592003375908412
  // 0.029487027326084, 0.798090501726892, -1.601626069307932
  return 0;
}
