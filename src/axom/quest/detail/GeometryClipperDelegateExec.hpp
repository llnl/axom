...
public:
  ...
  */
  ...
  void computeClipVolumes3D(axom::ArrayView<double> ovlap)
  {
    ...
    const bool useOcts = getGeometryClipperStrategy().getShapeAsOcts(geomAsOcts);
    const bool useTets = getGeometryClipperStrategy().getShapeAsTets(geomAsTets);
