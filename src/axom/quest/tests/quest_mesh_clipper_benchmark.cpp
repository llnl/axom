 ...
 *
 * Mesh and geometry construction are excluded from both timing modes. The
 * "warm" mode also excludes the first clip so it measures steady-state reuse,
 * while "cold" constructs a MeshClipper and its output for every iteration.
 * Device work is synchronized before an iteration ends. After timing, an
 * independent clip checks the result against a reference volume and reports
 * per-clip work counters.
 */
...
double expectedVolume(const std::string& shape)
{
  ...
  if(shape == "cone")
  {
    ...
    return PI * height *
      (baseRadius * baseRadius + baseRadius * topRadius + topRadius * topRadius) / 3.0;
  }
  if(shape == "tet")
  {
    constexpr double length = 1.55;
    return (16.0 / 15.0) * length * length * length;
  }
  if(shape == "hex")
  {
  ...
  if(shape == "tetmesh")
  {
    ...
    return 4.0 * length * length * length;
  }
  if(shape == "sor")
  {
    return 4.70237;
                  ...
                  const ClipProblem& problem)
{
  ...
  // for one clip, independent of benchmark iteration count and warm/cold mode.
  quest::experimental::MeshClipper validationClipper(*problem.shapeMesh, problem.strategy);
  axom::Array<double> validationOverlap;
  validationClipper.clip(validationOverlap);
  synchronize(policy);

  const double volume = checksum(validationOverlap);
  const double expected = expectedVolume(shape);
  const double relativeError = std::abs(volume - expected) / expected;
  constexpr double maxRelativeError = 0.01;
  if(!std::isfinite(volume) || relativeError > maxRelativeError)
  {
    const std::string message =
      axom::fmt::format("volume {} differs from reference {} by {:.3f}% (limit is {:.1f}%)",
                        volume,
                        expected,
                        100.0 * relativeError,
                        100.0 * maxRelativeError);
    state.SkipWithError(message.c_str());
    return;
  }

  state.counters["volume"] = volume;
  state.counters["volume_error_pct"] = 100.0 * relativeError;
  state.counters["cells"] = problem.shapeMesh->getCellCount();
  const conduit::Node& stats = validationClipper.getClippingStats();
     ...
     stats.has_path("clipsSum") && stats.has_path("clipsOn"))
  {
    ...
    {
      state.counters["adaptiveSubdivisions"] = (leafCount - rootCount) / 7.0;
...
void clipMeshWarm(benchmark::State& state,
                  const std::string& shape,
                  RuntimePolicy policy,
                  int resolution,
                  int refinementLevel)
{
  ...
  // Prime lazy allocations and caches. Only repeated clipping is timed.
  ...
  state.SetLabel("warm: setup and first-use allocation excluded");
  reportResult(state, shape, policy, *problem);
}

void clipMeshCold(benchmark::State& state,
                  const std::string& shape,
                  RuntimePolicy policy,
                  int resolution,
                  int refinementLevel)
{
  ...
  state.SetLabel("cold: MeshClipper construction and first-use allocation included");
  reportResult(state, shape, policy, *problem);
}
...
int main(int argc, char** argv)
{
  ...
  std::vector<int> resolutions {16, 51};
  int refinementLevel = 5;
  std::vector<std::string> modes {"warm", "cold"};
  const std::set<std::string> availableModes {"warm", "cold"};
  ...
  app.add_option("--refinements", refinementLevel)
    ->description("Refinement level for sphere and SOR geometries (at least 5)")
    ->each([](const std::string& value) {
      if(std::stoi(value) < 5)
      {
    ...
    ->capture_default_str();
  app.add_option("-m,--modes", modes)
    ->description("Timing modes: warm reuses a clipper; cold constructs one for every iteration")
    ->expected(-1)
    ->check(axom::CLI::IsMember(availableModes))
    ->capture_default_str();
      ...
      for(const auto& policyName : policies)
      {
        ...
        for(const auto& mode : modes)
        {
          const std::string name =
            axom::fmt::format("clipMesh/{}/{}_ares_{}_{}", mode, shape, resolution, policyName);
          if(mode == "warm")
          {
            ::benchmark::RegisterBenchmark(name.c_str(), [=](benchmark::State& state) {
              clipMeshWarm(state, shape, policy, resolution, refinementLevel);
            });
          }
          else
          {
            ::benchmark::RegisterBenchmark(name.c_str(), [=](benchmark::State& state) {
              clipMeshCold(state, shape, policy, resolution, refinementLevel);
            });
          }
        }
      }
