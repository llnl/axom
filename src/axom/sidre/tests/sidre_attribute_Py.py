...
g_protocol_saves_attributes = {
    "sidre_hdf5", "sidre_conduit_json", "sidre_json", "sidre_layout_json"
}
...
def test_iterate_attributes():
    ...
    for attr in ds.attributes():
        raise AssertionError(
            f"Expected no attributes, but found: {{id:{attr.getIndex()}, name:'{attr.getName()}'}}")
...
def test_save_load_group_with_attributes_new_ds():
    ...
    for protocol in g_protocols:
        ...
        assert ds1.getAttribute(g_name_color).getTypeID() == ds2.getAttribute(
            g_name_color).getTypeID()

        assert ds1.getAttribute(g_name_color).getDefaultNodeRef().value() == ds2.getAttribute(
            g_name_color).getDefaultNodeRef().value()
