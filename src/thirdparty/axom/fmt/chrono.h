...
AXOM_FMT_CONSTEXPR auto lossless_integral_conversion(const From from, int& ec)
    -> To {
  ec = 0;
...
template <typename To, typename From,
          AXOM_FMT_ENABLE_IF(!std::is_same<From, To>::value)>
AXOM_FMT_CONSTEXPR auto safe_float_conversion(const From from, int& ec) -> To {
  ec = 0;
  using T = std::numeric_limits<To>;
...
#ifdef AXOM_FMT_USE_UTC_TIME
// Use the provided definition.
#elif defined(__cpp_lib_chrono)
#  define AXOM_FMT_USE_UTC_TIME (__cpp_lib_chrono >= 201907L)
#else
#  define AXOM_FMT_USE_UTC_TIME 0
#endif
#if AXOM_FMT_USE_UTC_TIME
...
#ifdef AXOM_FMT_USE_LOCAL_TIME
// Use the provided definition.
#elif defined(__cpp_lib_chrono)
#  define AXOM_FMT_USE_LOCAL_TIME (__cpp_lib_chrono >= 201907L)
#else
#  define AXOM_FMT_USE_LOCAL_TIME 0
#endif
#if AXOM_FMT_USE_LOCAL_TIME
...
inline auto localtime_r AXOM_FMT_NOMACRO(...) -> null<> { return null<>(); }
...
void write_codecvt(codecvt_result<CodeUnit>& out, string_view in,
  ...
  AXOM_FMT_PRAGMA_CLANG(diagnostic ignored "-Wdeprecated")
...
AXOM_FMT_NORETURN inline void throw_duration_error() {
  AXOM_FMT_THROW(format_error("cannot format duration"));
}
 ...
 */
AXOM_FMT_DEPRECATED inline auto localtime(std::time_t time) -> std::tm {
  struct dispatcher {
    std::time_t time_;
    std::tm tm_;
...
#if AXOM_FMT_USE_LOCAL_TIME
template <typename Duration>
AXOM_FMT_DEPRECATED auto localtime(std::chrono::local_time<Duration> time)
    -> std::tm {
...
AXOM_FMT_CONSTEXPR inline auto get_units() -> const char* {
...
AXOM_FMT_CONSTEXPR auto parse_chrono_format(const Char* begin, const Char* end,
                                       Handler&& handler) -> const Char* {
  if (begin == end || *begin == '}') return begin;
  if (*begin != '%') AXOM_FMT_THROW(format_error("invalid format"));
  auto ptr = begin;
  while (ptr != end) {
    ...
    case 'E': {
      ...
      default:  AXOM_FMT_THROW(format_error("invalid format"));
      }
      break;
    }
    case 'O':
...
template <typename Derived> struct null_chrono_spec_handler {
  ...
  AXOM_FMT_CONSTEXPR void on_year(numeric_system, pad_type) { unsupported(); }
  ...
  AXOM_FMT_CONSTEXPR void on_dec_month(numeric_system, pad_type) { unsupported(); }
  AXOM_FMT_CONSTEXPR void on_dec0_week_of_year(numeric_system, pad_type) {
    unsupported();
  }
  AXOM_FMT_CONSTEXPR void on_dec1_week_of_year(numeric_system, pad_type) {
    unsupported();
  }
  AXOM_FMT_CONSTEXPR void on_iso_week_of_year(numeric_system, pad_type) {
    unsupported();
  }
  AXOM_FMT_CONSTEXPR void on_day_of_year(pad_type) { unsupported(); }
  AXOM_FMT_CONSTEXPR void on_day_of_month(numeric_system, pad_type) {
    unsupported();
  }
  ...
  AXOM_FMT_CONSTEXPR void on_utc_offset(numeric_system) { unsupported(); }
...
class tm_format_checker : public null_chrono_spec_handler<tm_format_checker> {
 ...
 public:
  ...
  AXOM_FMT_NORETURN inline void unsupported() {
    AXOM_FMT_THROW(format_error("no format"));
  }

  template <typename Char>
  AXOM_FMT_CONSTEXPR void on_text(const Char*, const Char*) {}
  AXOM_FMT_CONSTEXPR void on_year(numeric_system, pad_type) {}
  ...
  AXOM_FMT_CONSTEXPR void on_dec_month(numeric_system, pad_type) {}
  AXOM_FMT_CONSTEXPR void on_dec0_week_of_year(numeric_system, pad_type) {}
  AXOM_FMT_CONSTEXPR void on_dec1_week_of_year(numeric_system, pad_type) {}
  AXOM_FMT_CONSTEXPR void on_iso_week_of_year(numeric_system, pad_type) {}
  AXOM_FMT_CONSTEXPR void on_day_of_year(pad_type) {}
  AXOM_FMT_CONSTEXPR void on_day_of_month(numeric_system, pad_type) {}
  AXOM_FMT_CONSTEXPR void on_24_hour(numeric_system, pad_type) {}
  AXOM_FMT_CONSTEXPR void on_12_hour(numeric_system, pad_type) {}
  AXOM_FMT_CONSTEXPR void on_minute(numeric_system, pad_type) {}
  AXOM_FMT_CONSTEXPR void on_second(numeric_system, pad_type) {}
  ...
  AXOM_FMT_CONSTEXPR void on_utc_offset(numeric_system) {
    if (!has_timezone_) AXOM_FMT_THROW(format_error("no timezone"));
  }
...
template <typename T, AXOM_FMT_ENABLE_IF(has_tm_zone<T>::value)>
...
inline auto to_nonnegative_int(T value, Int upper) -> Int {
  if (!std::is_unsigned<Int>::value &&
    ...
    AXOM_FMT_THROW(format_error("chrono value is out of range"));
  }
...
void write_floating_seconds(memory_buffer& buf, Duration duration,
  ...
  AXOM_FMT_ASSERT(std::is_floating_point<rep>::value, "");
  ...
  axom::fmt::format_to(std::back_inserter(buf), AXOM_FMT_STRING("{:.{}f}"),
...
class tm_writer {
 private:
  ...
  template <typename T, AXOM_FMT_ENABLE_IF(has_tm_gmtoff<T>::value)>
...
struct chrono_format_checker : null_chrono_spec_handler<chrono_format_checker> {
  ...
  AXOM_FMT_NORETURN inline void unsupported() { AXOM_FMT_THROW(format_error("no date")); }
  ...
  AXOM_FMT_CONSTEXPR void on_iso_time() {}
  AXOM_FMT_CONSTEXPR void on_am_pm() {}
  AXOM_FMT_CONSTEXPR void on_duration_value() const {
    if (has_precision_integral)
      AXOM_FMT_THROW(format_error("precision not allowed for this argument type"));
  }
  AXOM_FMT_CONSTEXPR void on_duration_unit() {}
};

template <typename T,
          AXOM_FMT_ENABLE_IF(std::is_integral<T>::value&& has_isfinite<T>::value)>
...
class year_month_day {
 ...
 public:
  ...
  constexpr auto year() const noexcept -> axom::fmt::year { return year_; }
  constexpr auto month() const noexcept -> axom::fmt::month { return month_; }
  constexpr auto day() const noexcept -> axom::fmt::day { return day_; }
};
...
template <typename Char> struct formatter<std::tm, Char> {
 ...
 protected:
  ...
  AXOM_FMT_CONSTEXPR void set_localized() { specs_.set_localized(); }

  AXOM_FMT_CONSTEXPR auto do_parse(parse_context<Char>& ctx, bool has_timezone)
      -> const Char* {
    auto it = ctx.begin(), end = ctx.end();
    if (it == end || *it == '}') return it;
