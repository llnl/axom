...
#if AXOM_FMT_HAS_CPP_ATTRIBUTE(clang::lifetimebound)
#  define AXOM_FMT_LIFETIMEBOUND [[clang::lifetimebound]]
#else
#  define AXOM_FMT_LIFETIMEBOUND
#endif
AXOM_FMT_PRAGMA_CLANG(diagnostic error "-Wreturn-stack-address")
...
template <typename T, T... N> struct integer_sequence {
  ...
  static AXOM_FMT_CONSTEXPR auto size() -> size_t { return sizeof...(N); }
};
...
AXOM_FMT_CONSTEXPR void for_each(index_sequence<Is...>, Tuple&& t, F&& f) {
  using std::get;
...
AXOM_FMT_CONSTEXPR void for_each(Tuple&& t, F&& f) {
...
template <typename Char> struct parse_empty_specs {
  template <typename Formatter> AXOM_FMT_CONSTEXPR void operator()(Formatter& f) {
    f.parse(ctx);
...
struct formatter<Tuple, Char,
                 enable_if_t<axom::fmt::is_tuple_like<Tuple>::value &&
                             axom::fmt::is_tuple_formattable<Tuple, Char>::value>> {
 private:
...
struct range_formatter<
 ...
 private:
  ...
  template <typename Output, typename It, typename Sentinel, typename U = T,
            AXOM_FMT_ENABLE_IF(std::is_same<U, Char>::value)>
...
struct formatter<tuple_join_view<Tuple, Char>, Char,
 ...
 private:
  ...
  AXOM_FMT_CONSTEXPR auto do_parse(parse_context<Char>& ctx,
...
AXOM_FMT_CONSTEXPR auto join(const Tuple& tuple AXOM_FMT_LIFETIMEBOUND, string_view sep)
