...
- Core: Axom core memory allocation semantics and defaults have changed to be more consistent. Specifically, the default allocation stractegy for host allocations is now Axom's malloc routine regardless of whether Axom is configured with Umpire. Previously, it was the Umpire default HOST allocator when Axom is configured with Umpire. Also, there is separation between Axom default global allocator and Axom default host allocator. Both are used for CPU-accessible allocations; however, the global default is more clearly tied with Dynamic allocations, where the memory space is implied based on the execution context. Host-facing code paths have been updated to use that distinction consistently and new methods have been added to enable users to query memory-space and allocator availability. Please see Core component user documentation for more information.
